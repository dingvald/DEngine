...
{
	int actionCost = BASE_ACTION_COST;
	auto actorComp = registry.try_get<component::Actor>(actor);
	if (actorComp)
	{
		switch (type)
		{
	...
	registry.emplace_or_replace<component::action::SpendPoints>(actor, actionCost);
}
