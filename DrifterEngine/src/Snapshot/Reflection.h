...
namespace snapshot {
    ...
    namespace ReflectionFunctions {
        ...
        template<typename T>
        void doLoad(void* data, InputArchive archive)
        {
        ...
        template<typename T>
        void doSave(void const* data, OutputArchive archive)
        {
        ...
        template<typename T>
        void doEmplace(entt::handle handle, void* data)
        {
            auto& comp = *static_cast<T*>(data);
        ...
        template<typename T>
        bool doContains(entt::const_handle handle)
        {
            return handle.all_of<T>();
        }

        template<typename T>
        T& doGetComponent(entt::handle h)
        {
        ...
        template<typename T>
        T const& doGetConstComponent(entt::const_handle h)
        {
            auto* v = h.template try_get<T>();
            if (v) {
                return *v;
